...
class CalculatedMultiSerializer extends CalculatedSerializerBase{
  ...
  protected function add_overall_feedback_declaration(ImsQtiWriter $item, $question){
  	$result = $item->add_outcomeDeclaration_feedback(self::OVERALL_FEEDBACK);
    //$result->add_defaultValue()->add_value(self::OVERALL_FEEDBACK);
    return $result;
  }
  ...
  protected function add_overall_feedback(ImsQtiWriter $item, $question){
    $text = $this->translate_text($question->options->incorrectfeedback, self::FORMAT_HTML, $question);
    $item->add_modalFeedback(self::OVERALL_FEEDBACK, self::FEEDBACK_INCORRECT, 'show')->add_flow($text);
    
    $text = $this->translate_text($question->options->correctfeedback, self::FORMAT_HTML, $question);
    $item->add_modalFeedback(self::OVERALL_FEEDBACK, self::FEEDBACK_CORRECT, 'show')->add_flow($text);
    
    $text = $this->translate_text($question->options->partiallycorrectfeedback, self::FORMAT_HTML, $question);
  ...
  protected function add_interaction(ImsQtiWriter $body, $question){
    ...
    foreach($question->options->answers as $answer){
      ...
      $text = $this->translate_text($answer->answer, self::FORMAT_HTML, $question);

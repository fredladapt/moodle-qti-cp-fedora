...
class fedora_fs_search extends fedora_fs_folder{
	...
	public function __construct($title='', $term, $search_level, $start_date= NULL, $end_date = NULL, $owner = NULL, $hitPageSize=0, $offset=0, $sort='' ){
		if($title){
			$this->title = $title;
		}
		if($term){
	...
	public function query($fedora){
		...
		$objects = self::sparql_find($fedora, $term, $search_level, $start, $end, $owner, $hit, $offset, $sort);
